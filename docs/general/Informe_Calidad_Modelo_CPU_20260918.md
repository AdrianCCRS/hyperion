...
### 15.2 Modelo de potencia P(f) = P0 + c f^k (ajustado en pacca01)

Script `fase2_clasificador/analysis/cpu_power_model.py`; resultados en `docs/libro/datos/cpu_calidad_20260918/potencia/`. Ajuste por kernel (43 kernels, RMSE mediano 0.34 W): P0 mediana 85 W (rango 79-100 W, casi igual entre kernels), c mediana 18 W, k mediana 1.97 (~ f^2). La parte estática es el 84 % de la potencia a 3.2 GHz (62-94 % según el kernel); bajar a 0.8 GHz reduce la potencia solo 15 % (mediana; 5-35 %). Alfa crítico del modelo (mayor alfa para el que algún nivel bajo mejora el EDP): mediana 0.14, entre 0.07 y 0.33 (los compute densos tienen el mayor margen de potencia pero alfa ~1, así que nunca lo alcanzan). Es coherente con el umbral medido 0.22-0.23 de los kernels STREAM (el modelo con potencia media por corrida lo subestima un poco). Conclusión cuantitativa: el piso de ~85 W (paquete completo, no controlable con DVFS de núcleo) explica el resultado "no actuar".

### 15.3 `power_w` como entrada (plan §2.5), 2026-09-19

Potencia por intervalo uncore = energía RAPL válida (paquete + DRAM) de las ventanas del intervalo sobre su duración (`build_power_source.py`; 1 281 515 intervalos, 0.01 % sin potencia). Comparación LOFO, 5 semillas, mismo protocolo y modelo final, ejecutado en pacca01 (`cpu_quality_report --stages power_w`): sin `power_w` 0.7096 (sd 0.0073), con `power_w` 0.7048 (sd 0.0047). Sin mejora (diferencia dentro de una desviación estándar, ligeramente negativa). Decisión: no se incluye. Razón consistente con 15.2: a frecuencia dada la potencia de paquete está dominada por un piso de ~85 W casi igual en ambas clases, así que casi no discrimina fase. Datos: `docs/libro/datos/cpu_calidad_20260918/potencia/power_w.csv`.

### 15.4 Selección formal entre 6 y 12 variables (2026-09-19)

Regla fijada antes de mirar el resultado: se elige la variante de menor error, salvo que la diferencia pareada no se distinga de cero, en cuyo caso se elige por costo de inferencia y luego por parsimonia. Comparación en pacca01, LOFO, 5 semillas, XGBoost con pesos por celda, bootstrap de familias pareado (4000 remuestreos): exactitud balanceada por celda 0.7027 (6 variables) frente a 0.7096 (12), diferencia +0.0069, IC95 [-0.014, +0.029], P(diferencia <= 0) = 0.28. No se distinguen. Latencia de una fila (1 hilo, `predict_proba`): 6 variables p50 243 µs, p95 265 µs, p99 403 µs; 12 variables 243 / 266 / 404 µs. Idénticas: las seis interacciones no cambian el costo de inferencia del árbol y su cálculo previo son productos y cocientes de tasas ya disponibles. Ambas cumplen el presupuesto de latencia. Resultado: empate estadístico y de costo. Con la regla, correspondería la variante de 6 variables por parsimonia; se recomienda, sin embargo, conservar la de 12 ya congelada, exportada y documentada, porque cambiarla obliga a reescribir toda la sección de resultados sin ganancia medible. Decisión final pendiente del autor. Datos en `docs/libro/datos/cpu_calidad_20260918/seleccion/`.

## 17. Familias compute nuevas: prueba externa, reentrenamiento y política (2026-09-19)

**Campaña** `pacca_cpu_compute_sweep_20260919` (paccaA100, turbo desactivado, 10 niveles x 3 repeticiones): seis cargas RAJAPerf/OpenMP (edge3d, fir, mass3dpa, mat_mat_shared, pi_reduce, trap_int) y `ptrchase`. 210 corridas, todas aceptadas. Las seis salen compute (99.9-100 % de sus ventanas etiquetadas; 154 960 intervalos elegibles en total, sin contar calibración). Alfa de las seis: 0.975-1.0. `ptrchase` sale 100 % memory con alfa 0.216; se usa solo como sonda de la política, nunca en el clasificador (F1-CPU-004).

**Prueba externa (una sola evaluación, modelo final entrenado en las 24 familias, 5 semillas, umbral 0.5 y abstención 0.85).** Todas las clases verdaderas son compute (fracción de memory <= 0.12 %). Exactitud por familia: trap_int 0.9998, pi_reduce 0.9998, mat_mat_shared 0.930, mass3dpa 0.746, edge3d 0.692, fir 0.604 (sd entre semillas 0.20 en edge3d y 0.32 en fir). Media de las seis: 0.83. Con abstención, cobertura 0.38 (edge3d), 0.55 (mass3dpa), 0.78 (fir), 0.83 (mat_mat_shared) y 1.0 (pi_reduce, trap_int). Lectura: generaliza bien a bucles escalares simples y matmul por teselas, y falla en núcleos de elementos finitos y filtros, que se parecen a memory en contadores; refuerza que el límite es de información y de diversidad, no del modelo. `ptrchase` (memory verdadera, fuera del dominio del clasificador) se predice memory en 99.7 %.

**Reentrenamiento con las seis como familias propias (30 familias, LOFO, 5 semillas, misma configuración).** Exactitud balanceada por celda: 0.727 (sd 0.009) en las 30; sobre las 24 familias originales 0.713 (antes 0.709; recuperación de compute 0.646 frente a 0.59 pooled anterior); sobre las seis nuevas 0.779 (recuperación de compute 0.89; sus celdas memory son de 97 a 361 ventanas y hacen ruidosa esa métrica). Mejora sobre las 24 familias: +0.004, dentro del ruido; la ganancia visible está en compute. Hay que recordar que ya no son familias selladas después de esta evaluación, y que las seis provienen del mismo suite, por lo que su independencia es menor.

**Política con las siete cargas (28 kernels memory con `ptrchase`, 22 compute).** Sigue siendo no actuar en ambas clases. En compute, todos los niveles de F1 a F8 empeoran el EDP de forma significativa (p < 0.03), sin ninguna mejora significativa. `ptrchase`, con alfa 0.216, gana solo 1.0 % en F1 (0.9896 del EDP de REF): confirma cuantitativamente que aun con alfa en el umbral la ganancia es casi nula. Datos en `docs/libro/datos/cpu_calidad_20260918/externa/` y `politica/`.

## 18. Población final de 30 familias (2026-09-20)

Decisión: la población final del experimento CPU son las 30 familias (24 originales más seis compute RAJAPerf), 49 kernels, 1470 corridas, 1 309 755 intervalos elegibles (1 296 981 con todas las tasas definidas). La batería completa se repitió sobre esa matriz (`build_final_source.py`, `cpu_quality_report.py`, `cpu_book_data.py`, `feature_contract.py`; resultados en `docs/libro/datos/cpu_calidad_30fam/`, ejecutada en pacca01) y el libro se regeneró con esas cifras. Cifras principales: exactitud balanceada por celda 0.727 (IC95 0.670 a 0.803), F1 macro 0.782, exactitud 0.794, AUC 0.855, MCC 0.566; con abstención 0.85: cobertura 0.733, exactitud por celda 0.750. Cambios cualitativos respecto de las 24 familias: (1) la métrica por celda ya no es claramente más estable que las de volumen (sd 0.009 frente a 0.002 a 0.011); (2) diez celdas de menos de 100 intervalos pesan 17.9 % de la métrica (excluidas: 0.785); (3) el vector de 6 variables da 0.738 frente a 0.727 de las 12, diferencia -0.011, IC95 [-0.028, +0.003], latencia idéntica; se conservan las 12; (4) retirar la frecuencia baja 0.014 (IC95 [-0.027, -0.003]); (5) la búsqueda Optuna anidada empeora 0.736 a 0.696 (por familia -0.029, IC95 [-0.052, -0.010]); (6) ECE 0.113; (7) con la intensidad operacional como entrada, 0.9993. Política con 50 kernels: no actuar; P0 mediana 85 W (83 % de la potencia); techo con clasificador perfecto 2.5 % medio, 12.5 % máximo.

**Correcciones a lo dicho el 2026-09-19/20.** (a) La primera pasada con 30 familias no tenía defecto de datos: las filas de las familias nuevas sin etiqueta (las rechazadas) ya se descartaban al cargar, y las cifras coinciden con las de la pasada filtrada (0.7273). (b) La etapa `external` de esta batería no es una prueba externa (la fuente ya incluye esas familias); no se usa. La prueba externa válida es la de la sección 17 (modelo entrenado con las 24 originales, exactitud 0.60 a 0.9998 por familia) y solo figura en el libro como comentario sobre generalización en la versión previa; con las 30 familias en la población, el libro ya no la presenta.

### 18.1 Modelo serializado y umbral final (2026-09-20)

Reexportado con las 30 familias (`export_cpu_selective_candidate.py`, pacca01): `/home/latorresn/hyperion-results/final/models/cpu_final_30fam_20260920/cpu_xgboost_pmu_interactions.joblib`; metadatos en `docs/libro/datos/cpu_calidad_30fam/modelo_serializado_metadata.json`. Se ajustó con 42 014 intervalos (muestra con tope de 1000 por celda, tras retirar tasas indefinidas). El LOFO interno con cobertura mínima 0.60 propuso el umbral 0.90 (con las 24 familias había elegido 0.85). **Decisión del autor (2026-09-20): congelar 0.85**, porque con 0.90 la métrica por celda mejora solo 0.005 (0.750 a 0.755, dentro del ruido) y la F1 macro agrupada de 0.858 a 0.874, mientras la cobertura cae de 0.733 a 0.666: los intervalos con confianza entre 0.85 y 0.90 (6.7 % del total, exactitud alrededor de 0.7) pasan de decidirse a *revisar*. Es una decisión posterior al resultado y así se declara en el libro; el metadato registra `selected_threshold: 0.9` y `threshold_fixed_by_user: true`. Cifras finales con 0.85: cobertura 0.733, exactitud por celda 0.750, F1 macro agrupada 0.858, sensibilidad compute 0.771; abstenerse al azar con cobertura equivalente da 0.732. Las figuras se regeneraron en pacca01 y se revisaron una a una por solapamientos de leyendas.

## 19. Vector final de 6 variables y umbral por exactitud por celda (2026-09-21)

Registro completo en `Seguimiento_Cambios_Plan_Director.md`, entrada F2-CPU-006. Resumen: el vector final son las seis tasas base; se evalúan los 1 309 755 intervalos elegibles (los 12 774 con cero fallos de caché ya no se descartan); exactitud por celda 0.728 (0.738 con abstención 0.85, cobertura 0.722), F1 macro agrupada 0.760 (0.842). Umbral: la regla por exactitud por celda propone 0.90; se congela 0.85. Auditoría §2.5 del vector final: máximo |rho| 0.741, VIF máximo 7.3. Latencia en paccaA100: XGBoost p50 243 us, p99 404 us. Modelo exportado: `cpu_selective_candidate_base6_20260921` en el clúster. Datos en `docs/libro/datos/cpu_calidad_30fam/`. Las secciones 18 y 18.1 describen el modelo de 12 variables y quedan como historial.

## 20. Potencia estadística y refinamiento condicionado de la política memory-bound (2026-09-22)

Registro completo en `Seguimiento_Cambios_Plan_Director.md`, entrada F2-CPU-007. Resumen: la tabla clase-frecuencia (sección 19, `no_actuar` en ambas clases) se auditó con un análisis de potencia estadística. Con la familia (19 memory-bound) como unidad, el ruido de medición (sd relativa mediana 1.05 %) es 5 a 10 veces menor que la dispersión entre familias (2.75 % en F0 a 15.53 % en F3): la varianza es heterogeneidad física, no error instrumental. El efecto mínimo detectable al 80 % de potencia es 1.75 % (F0), 3.38 % (F1) y 6.05 % (F2); el efecto mínimo relevante de 1 % declarado antes era indetectable por diseño con 19 familias.

Dentro de memory-bound se identificó una señal condicional: familias con `stall_mem_ratio` (ya en el vector de 6) >= 0.30 ganan 3.03 % en F1 y 2.70 % en F2 en promedio; las de razón baja pierden 5-28 %. Se congeló la regla "actuar en F2 si `stall_mem_ratio`(REF) >= 0.30" antes de medir, y se probó en 11 familias memory-bound nuevas (kernels RAJAPerf ajenos a las 30 de entrenamiento; otros 10 kernels construidos resultaron ser la misma familia ya usada y se descartaron sin medir). El tamizaje en REF (paccaA100) dejó solo 6 de 11 familias del lado de actuar (objetivo: 8); se midió de todas formas por decisión del autor. Resultado del barrido REF/F0/F1/F2 x 3 repeticiones (132/132 corridas aceptadas): ganancia media del EDP en F2 sobre las 6 familias que actúan = -25.76 % (empeora), IC95 [-50.61 %, -6.84 %], con una sola familia (`apps_pressure`) mostrando la ganancia esperada (+3.5 %) y `algorithm_histogram` (razón de stall más alta, 0.97) empeorando 85.6 %. La regla no generalizó a familias nuevas.

**Cierre:** la política CPU queda sin cambios (`no_actuar` en ambas clases, sección 19). `policy_cpu.json` incorpora las claves `statistical_power_analysis` y `conditional_refinement_attempt_v1` con el detalle completo. El libro documenta el intento como hallazgo negativo con dirección conocida (§ Potencia del diseño y un intento de refinamiento, `03_resultados.tex`), no como limitación sin explorar. Datos y scripts: `fase2_clasificador/analysis/cpu_policy_by_family.py`, `cpu_policy_conditional.py`, `cpu_alpha_predict.py`; evidencia en `docs/libro/datos/cpu_calidad_30fam/politica/v1_prueba_sellada/`.
