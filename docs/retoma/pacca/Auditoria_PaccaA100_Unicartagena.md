...
- El acceso a contadores uncore se probó después, directo al syscall (sin depender del CLI `perf`, ausente aquí) — **confirmado bloqueado**, ver sección 6 (ARC-59, 2026-08-06).
...
**(2026-08-06, ARC-59) Confirmado bloqueado, con prueba directa en hardware real** — mismo patrón que la prueba PID+inherit de la sección 5, esta vez apuntando a un PMU uncore. Programa C mínimo (`test_perf_uncore.c`, no comprometido al repo, solo vivió en `$HOME` del nodo) que abre `perf_event_open` con `type=55` (`uncore_imc_0`, leído de `/sys/bus/event_source/devices/uncore_imc_0/type`), `config=0x0f04` (`cas_count_read`, `event=0x04,umask=0x0f`), `pid=-1, cpu=0` (cpumask válido de esa caja es `0,8`): **`FALLA errno=13 (EACCES)`**. El evento free-running equivalente (`type=67`, `read`, ya pre-escalado a MiB por el kernel) da el mismo `EACCES`. Segunda vía independiente, LIKWID 5.2.2 (`module load likwid/5.2.2`): `likwid-perfctr -C 0 -g MEM` corre sin error superficial y hasta imprime una tabla con `CAS_COUNT_RD`/`CAS_COUNT_WR` con valores numéricos pequeños que parecen plausibles a primera vista -- **engañoso**: con `-V 3` (debug) se ve que el daemon propio de LIKWID (`likwid-accessD`, en `.../likwid/5.2.2/sbin/`, **no está instalado con setuid/root en este cluster** -- corre como el usuario invocador) falla al leer los MSR de las cajas MBOX (`Got error 'failed to read/write register' from access daemon reading reg 0x63A...`), y la tabla de métricas agregada final confirma `Memory data volume [GBytes] = 0` en las 6 filas (lectura/escritura/total) -- los números "plausibles" de la tabla cruda son ruido de un registro que nunca se pudo armar, no una medición real. **Conclusión: uncore está bloqueado en pacca para un usuario sin privilegios, exactamente la misma categoría que bloqueó P4 en felix** (los eventos uncore/system-wide requieren `perf_event_paranoid<1` o `CAP_PERFMON`/`CAP_SYS_ADMIN`, y aquí `paranoid=2`). No hay ruta sin intervención del administrador (bajar `perfmon_paranoid`, otorgar `CAP_PERFMON` a un binario concreto, o desplegar `likwid-accessD` con setuid) -- la validación cruzada de `bytes_moved_window` contra un ground truth de bytes reales de DRAM **no es viable en pacca sin permisos adicionales**, igual que en felix. Ver `Informe_Diagnostico_Beta_2026-08-05.md` sección 6 para el sesgo de ~30-34% que esto deja sin poder cuantificar de forma independiente por ahora.
