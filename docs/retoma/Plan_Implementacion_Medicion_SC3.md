...
**Estado (2026-08-01):** primer `run_campaign_preflight()` real corrido contra felix (`delegated_cpus=0-5`, entorno Conda `hyperion-hpc` ya existente en el clúster). Encontró y corrigió tres bloqueadores reales (ARC-36/ARC-37): `_parse_cpu_list()` no entendía el formato real de `freqdomain_cpus` (afectaba E10, un check de seguridad), `catalog.yaml` le faltaba `estimated_memory_bytes` en las entradas de calibración (bloqueaba C05 siempre), y `pmc_count` nunca se implementó (bloqueaba D05 siempre — cerrado con `environment.probe_pmc_count()`, detección empírica real, felix soporta 5 contadores simultáneos). Con los tres corregidos, **el preflight completo queda en verde total** (42 checks, catálogo real de 8 kernels). Aún no se escribió el manifest real `campaign_felix_ref.yaml` (el catálogo de kernels va a cambiar — DGEMM y posibles ajustes — antes de fijarlo).
...
**Estado (2026-08-01): completo.** `campaign_felix_ref.yaml` corrió de punta a punta contra felix — 6/6 combinaciones aceptadas, 0 rechazadas. En el camino se encontraron y corrigieron tres bugs reales que nunca se habían ejercitado fuera de mocks (ver ARC-42/43/44): el launcher C++ nunca escribía el stdout del proceso medido en el caso exitoso (rompía CAL-02/CAL-03/POST-09 en silencio); `calibration.py` no convertía las unidades nativas del stdout (MB/s, GFLOP/s) antes de guardarlas, sesgando `i_ridge` por un factor de ~1000; y el check E06 (procesos ajenos) filtraba por `Cpus_allowed` en vez de por actividad real (`processor`+`state` de `/proc/stat`), rechazando el 100% de las combinaciones por daemons del sistema en reposo. `windows.csv` real generado por primera vez: señal físicamente coherente (`npb_ep` casi todo `intensity_undefined` porque casi no toca memoria — esperado, no un bug; `npb_mg` mayormente `memory_bound` con mezcla real de `compute_bound`). Detalle completo, incluidos los hallazgos menores no bloqueantes (advertencia D04, conteo de hilos de calibración, preflight no automático en el CLI), en ARC-45.
