...
La ecuación \eqref{eq:flops-medidos} cubre únicamente operaciones de \textbf{doble} precisión: los cuatro sub-eventos de precisión simple del mismo evento de PMU (\texttt{FP\_ARITH\_INST\_RETIRED}, umask \texttt{0x02}/\texttt{0x08}/\texttt{0x20}/\texttt{0x80}) no se abren, porque hacerlo excedería el presupuesto de diez contadores simultáneos ya agotado por completo. Esta decisión asume que las cargas del catálogo operan en doble precisión, coherente con la convención de las suites empleadas --- NAS Parallel Benchmarks declara sus variables aritméticas en doble precisión por diseño, y la rutina de álgebra lineal invocada es \texttt{cblas\_dgemm}, la variante de doble precisión de la interfaz BLAS, nunca su contraparte de precisión simple \texttt{cblas\_sgemm} --- pero esa convención no es, por sí sola, una medición. Para no dejarla sin contrastar, se repitió el procedimiento de validación anterior sustituyendo los cuatro sub-eventos de doble precisión por los de precisión simple, sobre los mismos dos kernels usados arriba: el kernel de cómputo denso registró exactamente cero en los cuatro contadores de precisión simple, y el kernel \textit{memory-bound} registró dos cuentas triviales en el sub-evento escalar (frente a un orden de $10^{10}$ operaciones de doble precisión en la misma corrida), consistentes con ruido de arranque y no con trabajo aritmético del propio kernel. Este contraste respalda la convención de doble precisión para los dos kernels verificados, pero no constituye una verificación exhaustiva de los seis kernels restantes del catálogo, que no fueron sometidos individualmente a este mismo contraste; de existir una carga que combine ambas precisiones, la fracción en precisión simple de su trabajo aritmético quedaría fuera del conteo de la ecuación \eqref{eq:flops-medidos}, sin que el instrumento lo señale como una anomalía.
