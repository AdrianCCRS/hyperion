...
La campaña DVFS de GPU se ejecutó sobre ocho cargas de conjunto de datos, dos niveles de frecuencia de CPU y seis de GPU, con tres repeticiones por combinación. El análisis posterior encontró que solo el 7.4\% de sus ventanas (160\,142 de 2\,171\,803) satisface simultáneamente los criterios de utilización mínima y presencia de etiqueta.

La causa dominante no es que la GPU registre utilización nula, sino que el 85.0\% de las ventanas \textbf{no contiene lectura alguna de GPU}. El origen es una diferencia de cadencia entre dos instrumentos: la ventana de telemetría de CPU tiene una duración mediana de 0.26~ms en estas cargas, mientras que el muestreo de la biblioteca de gestión de la GPU está configurado a intervalos de 100~ms. La GPU produce del orden de diez muestras por segundo frente a varios miles de cierres de ventana en el mismo lapso, de modo que la inmensa mayoría de las ventanas no puede contener una lectura fresca y queda con los campos de GPU indefinidos.

Se trata, por tanto, de un desajuste de diseño entre las granularidades de dos subsistemas de medición, no de un fallo del actuador ni del sensor: la energía y el reloj sí se registran correctamente en las ventanas que coinciden con una muestra. La implicación práctica es que el análisis de GPU debe realizarse a la granularidad de su propia telemetría, agregando ventanas hasta el intervalo de muestreo efectivo, y que esa reagregación puede efectuarse sobre los datos ya recolectados sin repetir la campaña.

Un segundo efecto, independiente del anterior, afecta a las ventanas que sí contienen lectura: aproximadamente la mitad reporta utilización por debajo del piso de ruido del sensor. En la carga \texttt{rodinia\_lud} los únicos valores observados son 0, 1 y 6\%, mientras la GPU consume una potencia media de 61~W sobre un reposo de aproximadamente 35~W, es decir, mientras está ejecutando trabajo real. La utilización reportada por la biblioteca de gestión no resulta, en esa carga, un indicador fiable de actividad; la potencia por encima del reposo lo es más. Este trabajo documenta la discrepancia pero no la resuelve: establecer cuál de las dos señales debe gobernar el criterio de aceptación exige una comparación controlada que no se ha realizado.
...
Séptima, la telemetría de GPU presenta un desajuste de cadencia respecto de la de CPU que deja el 85.0\% de las ventanas sin lectura del acelerador (sección \ref{sec:resultados-gpu-cadencia}). El análisis de GPU debe realizarse a la granularidad de su propio muestreo; adicionalmente, la utilización reportada por la biblioteca de gestión resulta un indicador poco fiable de actividad en al menos una carga, y este trabajo documenta la discrepancia sin resolver cuál de las señales disponibles debe gobernar el criterio de aceptación.
