...
The work is organised in four phases: workload characterisation and dataset construction; training and selection of a lightweight supervised model; implementation of the control service; and experimental validation against the native Linux governors. This document reports the measurement instrument and experimental protocol built during the first phase, together with a dual catalogue in which every compute configuration exists as a functionally equivalent CPU variant and GPU variant, allowing both devices to be compared over the same decision unit. Its central methodological contribution is a reproducible procedure for attributing time and energy to the true dispatch cost of an operation --- separating the cost paid once per process from the cost paid on every call --- with explicit traceability of the quality of each observation and direct verification of every instrument mechanism against the real state of the hardware.
...
\section{Fase 2: Desarrollo del modelo de selección}
\label{sec:fase2}

Esta fase construye el modelo que decide, para cada operación que la aplicación va a despachar, bajo qué configuración conviene ejecutarla. Las dos definiciones que determinan el resto del diseño --- sobre qué unidad se predice, y qué cantidad se predice sobre ella --- se establecen a continuación, apoyadas en la caracterización de la Fase 1 y no en una preferencia de diseño.
...
El \textbf{nivel 2} contiene una fila por configuración candidata. Se obtiene agregando las ventanas válidas de cada corrida sobre la región de despacho definida en la sección \ref{sec:contrato-coldwarm}, promediando después las repeticiones, y uniendo finalmente las variantes de CPU y de GPU de una misma configuración mediante un identificador compartido. Ese identificador es el que permite que una misma operación con un mismo tamaño de problema exista como dos filas comparables, una por dispositivo, y es lo que hace posible resolver la ecuación \eqref{eq:target-selector} sobre el producto $\mathcal{D}\times\mathcal{F}_d$ completo.

Una consecuencia de este diseño debe declararse explícitamente, porque determina el tamaño efectivo de la muestra: el número de ejemplos de entrenamiento es el número de configuraciones distintas, no el número de corridas ni el de ventanas. Presentar como tamaño muestral las decenas de miles de ventanas del nivel 1 sería incorrecto, dado que las ventanas de una misma corrida no son observaciones independientes entre sí ni constituyen decisiones distintas.

Puesto que el número de iteraciones que ejecuta cada variante se calibra por dispositivo para que ambas alcancen una duración medible comparable --- y por tanto no coincide entre la variante de CPU y la de GPU de una misma configuración ---, la agregación de nivel 2 normaliza tiempo y energía por iteración antes de cualquier comparación entre dispositivos. Omitir esa normalización invalidaría la etiqueta de la ecuación \eqref{eq:target-selector}, y por ello se implementa como paso obligatorio del constructor del conjunto y se verifica mediante prueba automatizada.

\subsection{Características de entrada y prevención de fuga}
\label{sec:fase2-features}

Las características que recibe el modelo se agrupan en tres familias, delimitadas por un criterio único: solo puede entrar aquello que estaría disponible \textit{antes} de ejecutar la configuración candidata.

La primera familia son los descriptores de la configuración, conocidos sin medir nada: la operación, su tamaño de problema, el dispositivo candidato y el estado de frecuencia candidato. La segunda son los descriptores estáticos de la plataforma, constantes en este nodo pero explicitados para que la formulación sea trasladable. La tercera es la telemetría de una \textbf{sonda}: una ejecución breve de la misma operación en un único estado de referencia, de la cual se derivan las métricas microarquitectónicas agregadas --- instrucciones por ciclo, tasa de fallos del último nivel de caché, fallos por cada mil instrucciones, fracción de ciclos de estancamiento, intensidad operacional medida y magnitudes energéticas.

La prohibición de fuga es explícita y verificable por prueba automatizada: ninguna característica puede provenir de la ejecución de la configuración candidata --- ni su tiempo, ni su energía, ni su potencia ---, pues constituirían la variable que el modelo debe predecir. Esta restricción es la misma que ya regía en el diseño anterior y se conserva sin relajarla.

\subsection{Protocolo de validación}
\label{sec:fase2-validacion}

Dado que el tamaño efectivo de la muestra es el número de configuraciones, y que las configuraciones de una misma operación a distintos tamaños comparten estructura algorítmica, una partición aleatoria a nivel de fila produciría una estimación optimista de la capacidad de generalización. La validación se realiza por tanto dejando fuera una operación completa en cada pliegue, de modo que el pliegue de prueba contenga únicamente configuraciones de una operación nunca vista en entrenamiento. Esta es la condición que el sistema enfrentaría en despliegue ante una aplicación cuyas fases no estuvieran representadas en el catálogo.

La línea base contra la cual se mide el aporte del modelo --- la mejor configuración constante única --- se selecciona utilizando exclusivamente los pliegues de entrenamiento. Elegirla observando el pliegue de prueba incorporaría a la línea base información que no tendría en despliegue e inflaría artificialmente el margen atribuido al modelo.

Se reportan, junto a las métricas de clasificación, dos magnitudes que expresan el resultado en los términos del cuarto objetivo específico: la pérdida de EDP respecto del óptimo alcanzable con conocimiento perfecto, y la latencia de inferencia medida.
...
Cuando la sonda descrita en la sección \ref{sec:fase2-features} no esté disponible por tratarse de la primera aparición de una operación, el agente opera bajo una estrategia de decisión estática construida únicamente sobre los descriptores de configuración; a partir de la primera ejecución real, la telemetría observada alimenta las decisiones siguientes sobre esa misma operación. Ambas estrategias se evalúan por separado en la Fase 4, dado que corresponden a situaciones de despliegue distintas.
...
Ambos microbenchmarks se compilaron y verificaron funcionalmente sobre la plataforma, y la campaña que los ejercita a lo largo de la rejilla de frecuencias ampliada se ejecutó íntegramente: 320 de 320 combinaciones aceptadas, sin rechazos. El primero confirma la predicción de diseño, según se reportó más arriba. El segundo produjo un resultado de otro orden y no directamente comparable: sus tres variantes muestran $\alpha \approx 0.002$--$0.003$, muy por debajo incluso del microbenchmark de persecución de punteros puro, pero su duración total permanece prácticamente constante entre el estado de máximo y mínimo reloj (variación menor al 1\%) --- consecuencia esperada del propio diseño, que fija la duración de cada fase por tiempo y no por trabajo (párrafo anterior). Ese $\alpha$ describe, por tanto, una propiedad del instrumento de control experimental y no evidencia de que una carga real pueda alcanzar una insensibilidad al reloj de esa magnitud; su función en este trabajo es la de control positivo --- confirma que el instrumento detecta correctamente un régimen \textit{memory-bound} extremo cuando existe --- y de fuente de la etiqueta de referencia independiente descrita en el párrafo anterior, no la de un tercer candidato de catálogo.
...
El trabajo inmediato consiste en cerrar la adquisición del conjunto de datos del selector y ejecutar sobre él las tres fases restantes.

Primero, completar la campaña del catálogo dual. El eje de CPU está cerrado: la totalidad de sus combinaciones se ejecutó y aceptó bajo el contrato temporal de la sección \ref{sec:contrato-coldwarm}, con verificación por corrida de las fronteras cold/warm, de la frecuencia efectivamente aplicada en cada nivel y de la presencia de las tres fuentes energéticas. El eje de GPU, cuyo producto cartesiano de frecuencias de ambos dispositivos lo hace considerablemente más costoso, se ejecuta en sesiones sucesivas sobre un mismo directorio de salida, reanudando desde las combinaciones ya aceptadas. La auditoría por sesión, y no solo al final, es parte del procedimiento: un rechazo sistemático concentrado en un nivel intermedio debe detener la campaña y diagnosticarse, en lugar de relajar retrospectivamente los márgenes para que los datos pasen.

Segundo, construir el conjunto de datos de nivel 2 y entrenar el modelo de selección. Ese constructor debe agregar por identificador de configuración, restringir la integración de tiempo y energía a la región de despacho, normalizar por iteración antes de comparar dispositivos --- puesto que el número de iteraciones se calibra por dispositivo y no coincide entre las dos variantes de una misma configuración --- y resolver la ecuación \eqref{eq:target-selector} sobre el producto de dispositivos y frecuencias. La validación dejando fuera una operación completa en cada pliegue (sección \ref{sec:fase2-validacion}) es la que determinará si la formulación generaliza hacia operaciones no observadas, que es la pregunta que decide su utilidad práctica.

Tercero, implementar el agente y evaluarlo. La comparación de cuatro escenarios descrita en la sección \ref{sec:fase4} --- ejecución íntegra en CPU, íntegra en GPU, guiada por el selector, y guiada por un oráculo con conocimiento posterior --- es la que permite atribuir un resultado insuficiente a la ausencia de margen en la plataforma o a la incapacidad del modelo para capturarlo, distinción que sin el cuarto escenario resultaría ambigua. La medición del umbral de amortización del costo de decisión responde directamente a la cláusula de la pregunta de investigación relativa al \textit{overhead} de la inferencia.

Más allá del alcance comprometido, dos extensiones quedan abiertas. La primera es la política de repeticiones adaptativa: la evidencia recogida sobre convergencia del coeficiente de variación indica que tres repeticiones son un mínimo operativo defendible para la mayoría de las operaciones del catálogo, pero no para todas, y una política que escale las repeticiones únicamente donde el margen entre las dos mejores configuraciones sea estrecho o la dispersión observada lo exija aprovecharía mejor el tiempo de cómputo que un valor uniforme. La segunda es la extensión del criterio de calidad por ventana al eje del acelerador: el instrumento registra por muestra el reloj y la temperatura del dispositivo, pero no los emplea todavía como compuerta automática, de modo que una eventual limitación térmica durante una sesión prolongada se detectaría hoy mediante análisis posterior y no durante la propia campaña.
