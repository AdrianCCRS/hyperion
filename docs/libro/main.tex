...
Ninguno de los dos términos de la ecuación \eqref{eq:intensidad} se observa de forma directa y exacta mediante contadores de propósito general, lo que introduce una consideración metodológica relevante.

En cuanto al numerador, la disponibilidad y la semántica de los eventos de hardware capaces de aproximar operaciones de punto flotante no son uniformes entre microarquitecturas: dependiendo del procesador concreto, algunos de los eventos disponibles contabilizan instrucciones, micro-operaciones ejecutadas o eventos especulativos en lugar de operaciones arquitectónicamente retiradas, lo que puede introducir sobreconteo o discrepancias respecto al volumen efectivo de trabajo de punto flotante --- un problema documentado empíricamente en contadores de esta naturaleza \cite{Weaver2008}. Herramientas de perfilado establecidas construyen métricas de FLOPs precisamente a partir de eventos de PMU específicos de cada microarquitectura --- así procede, por ejemplo, LIKWID a través de sus grupos de eventos predefinidos como \texttt{FLOPS\_DP} \cite{Treibig2010}, y de forma análoga las métricas de actividad de punto flotante e \textit{instruction mix} documentadas por Intel VTune \cite{VTune2026} --- mientras que interfaces de abstracción como PAPI ofrecen una capa portable cuyo soporte efectivo continúa dependiendo del hardware subyacente \cite{PAPI2026}. Esta vía --- medir el evento correcto, por ventana, en la microarquitectura concreta --- exige que dicho evento esté disponible y correctamente mapeado, condición que, de acuerdo con lo expuesto en la sección \ref{sec:multiplexacion}, no siempre se cumple, y que además compite por el presupuesto ya ajustado de contadores físicos simultáneos descrito en la Tabla \ref{tab:contadores}.

Ante esta restricción, el presente trabajo no mide directamente los FLOPs de cada ventana, sino que los \textbf{estima} a partir de una única cifra: el conteo total de operaciones que el propio programa reporta al finalizar toda la corrida --- una práctica presente en diversas suites de benchmarks científicos, en las que el volumen de trabajo se deriva analíticamente del problema resuelto (por ejemplo, a partir del número de operaciones de un método de álgebra lineal densa o de las iteraciones de un esquema numérico). Dicho total se reparte entre las ventanas de la corrida en proporción a la fracción de instrucciones retiradas observada en cada una:
...
El uso del símbolo $\widehat{\text{FLOPs}}_{i}$, en lugar de $\text{FLOPs}_{i}$, es deliberado: distingue notacionalmente el valor \textit{estimado} para la ventana $i$ de un eventual valor medido u observado por hardware, y se mantiene así en el resto del documento para evitar sugerir una igualdad que no se sostiene. Definiendo la densidad media global de operaciones de punto flotante por instrucción retirada como $\rho_{\text{global}} = \text{FLOPs}_{\text{total}} / \text{instrucciones}_{\text{total}}$, la ecuación \eqref{eq:prorrateo} puede reescribirse como $\widehat{\text{FLOPs}}_{i} = \rho_{\text{global}} \cdot \Delta\text{instrucciones}_{i}$. Esta reformulación hace explícita la hipótesis real del procedimiento: no es que las instrucciones sean un buen proxy de los FLOPs en un sentido vago, sino específicamente que la densidad real de FLOPs por instrucción retirada de cada ventana, $\rho_{i} = \text{FLOPs}_{i} / \Delta\text{instrucciones}_{i}$ (magnitud no observada en la ruta principal), se aproxima a la densidad promedio de toda la corrida, $\rho_{i} \approx \rho_{\text{global}}$. Esta distinción es importante porque una instrucción retirada y una operación de punto flotante no son la misma unidad: una instrucción puede no producir ningún FLOP (una carga, un salto, un cálculo de dirección), producir uno, o producir varios simultáneamente cuando opera sobre registros vectoriales o combina multiplicación y suma en una sola instrucción fusionada, de modo que instrucciones nunca debe leerse como sustituto directo de FLOPs, sino únicamente como el peso con el que se reparte un total ya conocido.

El criterio de reparto elegido no es indiferente frente a la alternativa de prorratear por tiempo transcurrido: esta última puede asignar una fracción elevada de FLOPs a ventanas de larga duración aunque dicha duración provenga de latencias, estancamientos o espera por memoria, y no de una mayor cantidad de trabajo aritmético, sesgando la intensidad en la dirección contraria a la que se desea discriminar. El número de instrucciones dinámicamente ejecutadas, en cambio, cuenta con antecedente como medida de progreso de un programa independiente del tiempo de reloj: la técnica SimPoint, empleada en caracterización de fases de programas, define y pondera regiones de ejecución precisamente en función de instrucciones dinámicas y no de intervalos de tiempo \cite{Hamerly2005}. Debe señalarse, sin embargo, que dicho antecedente respalda el uso de instrucciones como coordenada de progreso, no la proporcionalidad entre FLOPs e instrucciones: SimPoint existe justamente porque el comportamiento de un programa cambia entre fases, por lo que dos regiones con igual número de instrucciones pueden tener una composición (\textit{instruction mix}) muy distinta entre sí \cite{Hamerly2005}.

En consecuencia, no se identificó en la literatura consultada un antecedente que valide, como procedimiento estándar de estimación de intensidad operacional, el reparto de un total de FLOPs entre ventanas temporales mediante la fracción de instrucciones retiradas como peso; la práctica documentada en guías de referencia de Roofline experimental, como la de NERSC, es medir o derivar los FLOPs de la región analizada directamente, no reconstruirlos desde un total agregado \cite{NERSC2026}. El procedimiento aquí adoptado se presenta, por tanto, como un estimador específico de este instrumento, construido sobre dos elementos ya establecidos en la literatura --- el uso de instrucciones dinámicas como medida de progreso de la ejecución \cite{Hamerly2005} y la existencia de una densidad reconocible de operaciones de punto flotante por instrucción o \textit{instruction mix} \cite{VTune2026} --- a los que se añade una hipótesis adicional, específica de este instrumento y no establecida por los antecedentes consultados, de homogeneidad temporal de dicha densidad dentro de la región medida. El valor $\widehat{\text{FLOPs}}_{i}$ por ventana, y en consecuencia la intensidad operacional derivada de él en la ecuación \eqref{eq:intensidad}, debe entenderse como una magnitud estimada bajo ese supuesto, y no como una lectura directa de hardware ni como una medición Roofline estándar.

Cabe señalar que sí existe, en la literatura de instrumentación binaria, un procedimiento capaz de reconstruir el conteo dinámico de operaciones de punto flotante por ventana de forma mucho más directa que el prorrateo aquí adoptado, sin apoyarse en el supuesto de homogeneidad anterior: herramientas como PEBIL combinan un análisis estático del binario, que identifica de forma determinista las instrucciones de punto flotante contenidas en cada bloque básico del programa, con un conteo dinámico ligero de cuántas veces se ejecuta cada bloque durante la corrida, de modo que los FLOPs de una ventana se aproximan mediante la suma de ese producto sobre los bloques ejecutados en ella \cite{PEBIL2010}. Esta aproximación estructural es considerablemente más precisa que el prorrateo por densidad global, pero no debe describirse como una medida exacta de FLOPs: convertir instrucciones de punto flotante estáticas en un conteo de operaciones todavía depende de factores como el ancho vectorial, la fusión de multiplicación y suma, y el enmascaramiento de carriles en instrucciones SIMD, cuyo comportamiento efectivo puede variar en tiempo de ejecución según los datos procesados. Este método no se adoptó en el presente trabajo porque exige instrumentar y desensamblar el binario de cada carga, mientras que el catálogo experimental se compone de suites de benchmarks de terceros empleadas sin modificación de su código fuente ni de su binario compilado, según lo declarado en la sección de metodología; incorporar dicha instrumentación queda planteado como una vía de verificación más precisa para trabajo futuro, y no como parte del instrumento actual.

Esta hipótesis resulta más plausible en el presente trabajo que en el caso general de una aplicación HPC completa, porque la ventana de medición se restringe a la región estacionaria y repetitiva del kernel --- excluyendo asignación de memoria, creación de hilos y el intervalo de calentamiento inicial, conforme al criterio de exclusión descrito en la sección \ref{sec:calentamiento} --- y no a una ejecución con múltiples fases heterogéneas. Aun así, el supuesto no se da por válido sin verificación: su comprobación empírica, contrastando esta estimación contra eventos de punto flotante medidos directamente por hardware cuando la microarquitectura del nodo lo permita, se plantea como una campaña de calibración independiente de la adquisición principal, descrita en la sección \ref{sec:validacion-flops}.
...
\midrule
Procesador & 2 $\times$ Intel Xeon Gold 5315Y (Ice Lake-SP) \\
...
\midrule
Instrucciones retiradas & Base para IPC, tasa de instrucciones y prorrateo de operaciones. \\
...
El procedimiento de estimación de la intensidad operacional descrito en la sección \ref{sec:intensidad} --- prorrateo de un total de FLOPs reportado por el propio programa, en proporción a las instrucciones retiradas de cada ventana --- se diseñó para el dominio CPU, donde el presupuesto de contadores físicos simultáneos, discutido en la sección \ref{sec:multiplexacion}, impide incorporar de forma sostenida un evento de punto flotante junto con el resto de señales de la Tabla \ref{tab:contadores}. En el dominio GPU esta restricción no aplica de la misma manera: la intensidad operacional de cada carga se determina, en cambio, mediante perfilado directo con un depurador de instrucciones a nivel de kernel (NVIDIA Nsight Compute) \cite{Nsight2026}, que expone contadores de hardware capaces de contabilizar, por cada lanzamiento de kernel, tanto el tráfico total hacia memoria del dispositivo como el número de instrucciones aritméticas de punto flotante efectivamente ejecutadas, discriminadas por tipo de operación (suma, multiplicación y multiplicación--suma fusionada) y por precisión (simple o doble). A partir de estos conteos, la intensidad operacional de la carga se obtiene de forma directa como el cociente entre el total de operaciones de punto flotante --- contando cada multiplicación--suma fusionada como dos operaciones --- y el total de bytes transferidos, agregados sobre un número fijo de lanzamientos consecutivos del kernel.

Esta medición se realiza una única vez por carga, de forma independiente de la campaña de adquisición de telemetría, y su resultado se declara como una propiedad fija del catálogo, análoga a la suma de verificación del binario: al igual que ocurre en CPU, la intensidad operacional de un kernel es una propiedad de su algoritmo y del tamaño del problema que resuelve, no del estado de frecuencia bajo el cual se ejecuta, por lo que no es necesario --- ni válido, dado el costo de intrusión del perfilado detallado sobre el tiempo de ejecución --- repetir esta medición dentro de cada corrida de la campaña principal. A diferencia del estimador de la sección \ref{sec:intensidad}, que descansa sobre un supuesto de homogeneidad no verificado en el caso general, esta vía constituye una medición directa de ambos términos de la ecuación \eqref{eq:intensidad} y no requiere, por tanto, la validación adicional descrita en la sección \ref{sec:validacion-flops}.
...
\subsection{Validación del estimador de FLOPs por ventana}
\label{sec:validacion-flops}

El estimador de FLOPs por ventana descrito en la sección \ref{sec:intensidad} descansa sobre el supuesto de que la densidad de operaciones de punto flotante por instrucción retirada, $\rho_{i}$, se mantiene aproximadamente constante e igual a la densidad promedio de toda la corrida, $\rho_{\text{global}}$. Dicho supuesto no se da por válido sin verificación: se plantea una campaña de calibración, independiente de la adquisición principal, orientada a cuantificar su error.

El procedimiento previsto consiste en, para cada kernel del catálogo y cuando la microarquitectura del nodo disponga de un evento de punto flotante correctamente mapeado, repetir la corrida incorporando ese evento al conjunto de contadores adquiridos, de modo que se obtenga una medida directa de FLOPs por ventana, $\text{FLOPs}_{i}^{\text{HW}}$, además de las instrucciones retiradas de esa misma ventana. A partir de ambas series se calculará la densidad real por ventana $\rho_{i}^{\text{HW}} = \text{FLOPs}_{i}^{\text{HW}} / \Delta\text{instrucciones}_{i}$ y se contrastará contra $\rho_{\text{global}}$ mediante el coeficiente de variación de $\rho_{i}^{\text{HW}}$ a lo largo de la corrida, el error porcentual absoluto medio entre $\widehat{\text{FLOPs}}_{i}$ (estimado según la ecuación \eqref{eq:prorrateo}) y $\text{FLOPs}_{i}^{\text{HW}}$, y un ajuste lineal entre $\Delta\text{instrucciones}_{i}$ y $\text{FLOPs}_{i}^{\text{HW}}$, cuya pendiente el estimador supone igual a $\rho_{\text{global}}$ y cuyo intercepto supone cercano a cero. Una baja variabilidad de $\rho_{i}^{\text{HW}}$ respaldaría empíricamente el supuesto para el kernel y la plataforma evaluados; una variabilidad alta indicaría que el estimador debe restringirse a las cargas donde se verifique, o sustituirse por una medición directa cuando el presupuesto de contadores lo permita.

Esta campaña no se incluye en el conjunto mínimo de contadores de la adquisición principal, en atención a la restricción de presupuesto de contadores físicos simultáneos discutida en la sección \ref{sec:multiplexacion}; el evento de punto flotante se incorpora únicamente en corridas dedicadas de calibración, cuyo resultado se documenta de forma independiente y no sustituye la anotación de calidad de las corridas de la campaña principal.
