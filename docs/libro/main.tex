...
Este resultado no se sobrevende. En \texttt{cpu\_ready} y \texttt{none\_ready} la formulación por curva no aporta (ahorro global $-$1,5\,\% sin compuerta), igual que la formulación categórica; la compuerta de abstención sigue siendo necesaria en esos estados. La precisión de acción exacta se mantiene baja ($\approx 0{,}10$) pese al ahorro alto, porque varias acciones son casi equivalentes en EDP --- el ahorro capturado, no el acierto de acción puntual, es la métrica que importa aquí. El mapeo de nivel de frecuencia a fracción relativa usa un supuesto ($f_{\min}=0{,}35$) verificado solo indirectamente (REF/F0 con razón de tiempo mediana 1,0001 en CPU, consistente con que el gobernador nativo corre al máximo bajo carga) y pendiente de contrastar contra las frecuencias reales reportadas por el hardware. La integración al pipeline con compuerta de abstención, pruebas de regresión y su propia enmienda de protocolo queda como trabajo inmediato siguiente, registrada en el plan de reformulación.
