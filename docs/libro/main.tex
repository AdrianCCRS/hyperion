...
La disponibilidad y la semántica de los eventos PMU asociados a la aritmética de punto flotante no son uniformes entre microarquitecturas. Dependiendo del procesador, un evento puede contabilizar instrucciones arquitectónicamente retiradas, micro-operaciones ejecutadas o actividad especulativa, por lo que su valor no debe interpretarse automáticamente como un número de FLOPs. La distinción entre ejecución y retiro es relevante porque los procesadores fuera de orden pueden ejecutar instrucciones especulativamente y descartarlas después, por ejemplo tras una mala predicción de salto: un evento basado en ejecución puede contabilizar ese trabajo transitorio, mientras que un evento de retiro se aproxima al volumen de trabajo arquitectónicamente completado --- una fuente de sobreconteo documentada empíricamente en contadores de esta naturaleza \cite{Weaver2008}. Por esta razón, en este trabajo ningún evento PMU utilizado para derivar FLOPs se considera válido por defecto: su encoding se determina para la microarquitectura concreta, su semántica se documenta explícitamente, y su interpretación se contrasta empíricamente mediante microbenchmarks de resultado conocido, siguiendo el mismo criterio ya aplicado en este instrumento para otros contadores de comportamiento no uniforme entre plataformas (sección \ref{sec:paranoid}).

Para la plataforma experimental de este trabajo (Intel Xeon Gold 5315Y, Ice Lake-SP), la familia de eventos \texttt{FP\_ARITH\_INST\_RETIRED} utiliza el \textit{EventSelect} \texttt{0xC7}; este código identifica únicamente la familia de evento, y cada modalidad de operación aritmética se distingue además mediante su \textit{Unit Mask} (\texttt{UMask}), de modo que el encoding completo de cada sub-evento no se reduce a \texttt{0xC7} por sí solo. Los cuatro sub-eventos de doble precisión empleados en este trabajo se distinguen por el ancho del registro vectorial que ejecuta la instrucción --- escalar, y empaquetado de 128, 256 y 512 bits (no bytes: 512 bits equivalen a 64 bytes) --- mediante los \texttt{UMask} \texttt{0x01}, \texttt{0x04}, \texttt{0x10} y \texttt{0x40} respectivamente.

Estos eventos contabilizan instrucciones aritméticas de punto flotante retiradas, no FLOPs de forma abstracta: convertir su conteo a operaciones requiere considerar tanto el número de elementos de doble precisión procesados por instrucción (1, 2, 4 y 8 para los anchos escalar, 128, 256 y 512 bits, respectivamente) como el tipo de operación ejecutada. Para instrucciones elementales (\texttt{ADD}, \texttt{MUL}, entre otras) esto corresponde directamente a 1, 2, 4 y 8 FLOPs por instrucción retirada. Las instrucciones \textit{fused multiply-add} (FMA) realizan una multiplicación y una suma por elemento procesado --- el doble de trabajo aritmético que una instrucción elemental del mismo ancho --- pero esta distinción no exige una ponderación adicional en la fórmula: la propia definición del evento en la microarquitectura hace que una instrucción FMA incremente el contador en 2 por cada elemento procesado, en vez de 1, precisamente para que su cuenta ya represente operaciones de punto flotante y no instrucciones retiradas en bruto \cite{IntelPerfmon2026}. La Tabla \ref{tab:fma-pesos} resume esta relación.

\begin{table}[h]
\centering
\caption{FLOPs por instrucción retirada según ancho vectorial y tipo de operación, en el evento \texttt{FP\_ARITH\_INST\_RETIRED} de Ice Lake-SP. La columna FMA ya está incorporada en el valor que reporta el propio contador de hardware --- no debe aplicarse una ponderación adicional sobre ella.}
\label{tab:fma-pesos}
\begin{tabular}{@{}lccc@{}}
\toprule
...
Como estos eventos cuentan instrucciones y no FLOPs de forma abstracta, pero su propio valor de retiro ya incorpora el factor de FMA descrito arriba, el total de FLOPs medidos en una ventana se obtiene ponderando cada sub-evento únicamente por el número de elementos de doble precisión de su ancho vectorial:
...
donde cada $\Delta N$ es, como en el resto de contadores acumulativos de este instrumento, la diferencia entre dos lecturas consecutivas del contador de hardware para la ventana $i$ (sección \ref{sec:multiplexacion}) --- nunca su valor absoluto, que es acumulativo desde que la PMU se habilita.

El encoding crudo de los cuatro sub-eventos (sin alias simbólico expuesto por el kernel de Linux en este nodo, por lo que se programan como evento crudo o \textit{raw} mediante \texttt{perf\_event\_open}, con el encoding específico de Ice Lake-SP) se determinó a partir de la definición oficial de Intel para esta microarquitectura \cite{IntelPerfmon2026}, corroborada contra la tabla de eventos de LIKWID, que documenta explícitamente el grupo \texttt{FLOPS\_DP} con esta misma ponderación para Ice Lake-SP \cite{Treibig2010} --- en lugar de inferirse a partir de documentación genérica de otras microarquitecturas. Herramientas de perfilado como LIKWID e Intel VTune \cite{VTune2026}, o interfaces de abstracción como PAPI \cite{PAPI2026}, construyen sus propias métricas de FLOPs sobre estos mismos eventos, con soporte efectivo que continúa dependiendo de los eventos que la PMU concreta soporte y no de la interfaz de más alto nivel que los expone.

Incorporar estos cuatro contadores exige presupuesto: junto con los seis eventos ya establecidos para las demás señales de la Tabla \ref{tab:contadores}, agotan por completo el presupuesto de diez contadores físicos simultáneos verificado empíricamente en esta plataforma (sección \ref{sec:multiplexacion}), sin margen para eventos adicionales. Antes de adoptar la medición directa como fuente principal de FLOPs, se verificó empíricamente --- no se asumió --- que esta combinación de diez contadores abre sin inducir multiplexación (razón tiempo-contando/tiempo-habilitado igual a 1 en los diez, en corridas de prueba dedicadas) y que los valores resultantes son físicamente plausibles. La validación consistió en contrastar la ecuación \eqref{eq:flops-medidos} contra el total analítico de FLOPs de un kernel de referencia (multiplicación de matrices densas, invocando \texttt{cblas\_dgemm}, cuyo volumen de trabajo se conoce exactamente como $2 \cdot \text{iteraciones} \cdot n^{3}$): el error relativo fue de 0.29\% sin fijar afinidad de núcleos y de 0.30\% bajo la afinidad real de campaña (núcleos delegados fijados, hilos ajustados al mismo número de núcleos). Este resultado es, además, evidencia empírica indirecta de que la semántica de FMA descrita arriba (Tabla \ref{tab:fma-pesos}) está correctamente incorporada por el propio contador de hardware: \texttt{cblas\_dgemm} ejecuta su volumen de trabajo casi enteramente mediante instrucciones FMA, así que un error de ponderación en ese factor --- por defecto o por exceso --- se manifestaría como una discrepancia de un orden de magnitud cercano al doble, no como un error del 0.3\%. Como segundo contraste, sobre un kernel de régimen opuesto (\textit{memory-bound}, multi-hilo) el error relativo frente al total que el propio kernel reporta fue de 7.48\%, diferencia explicada por que el tiempo que dicho kernel cronometra internamente excluye su propia fase final de verificación de resultado --- que también ejecuta operaciones de punto flotante --- y no por un error del contador. Confirmada la viabilidad, la medición directa se incorporó como fuente principal de FLOPs por ventana en el instrumento, y se verificó a la escala completa de una campaña real (66 corridas, siete kernels, matriz declarada a seis estados de frecuencia --- REF y F0-F4): de aproximadamente 1.29 millones de ventanas con delta válido, el 100\% obtuvo su valor de FLOPs por esta vía, sin que ninguna arrojara un contador degradado. Esta validación es independiente de si la frecuencia solicitada por cada nivel llegó a aplicarse realmente sobre el hardware --- en la campaña citada, el permiso administrativo de escritura de frecuencia aún no se había concedido, por lo que las seis combinaciones se ejecutaron en la práctica sobre la misma frecuencia nativa del nodo; la medición de FLOPs por hardware, que es lo que aquí se valida, no depende del estado de frecuencia bajo el cual corre el kernel.

La ecuación \eqref{eq:flops-medidos} cubre únicamente operaciones de \textbf{doble} precisión: los cuatro sub-eventos de precisión simple del mismo evento de PMU (\texttt{FP\_ARITH\_INST\_RETIRED}, umask \texttt{0x02}/\texttt{0x08}/\texttt{0x20}/\texttt{0x80}) no se abren, porque hacerlo excedería el presupuesto de diez contadores simultáneos ya agotado por completo. Esta decisión descansa en que las cargas del catálogo de CPU operan en doble precisión, coherente con la convención de las suites empleadas --- NAS Parallel Benchmarks declara sus variables aritméticas en doble precisión por diseño, y la rutina de álgebra lineal invocada es \texttt{cblas\_dgemm}, la variante de doble precisión de la interfaz BLAS, nunca su contraparte de precisión simple \texttt{cblas\_sgemm} --- pero esa convención no se dio por válida sin contrastarla: se repitió el procedimiento de validación anterior sustituyendo los cuatro sub-eventos de doble precisión por los de precisión simple, aplicado esta vez a los nueve kernels de CPU del catálogo experimental (siete de conjunto de datos y dos de calibración), no solo a los dos empleados en la validación principal. En los nueve casos, los cuatro contadores de precisión simple registraron cero o, cuando mucho, una o dos cuentas triviales en el sub-evento escalar --- frente a órdenes de $10^{9}$ a $10^{10}$ operaciones de doble precisión en la misma corrida --- consistentes con ruido de arranque y no con trabajo aritmético del propio kernel. Con cobertura completa del catálogo de CPU, la precisión simple no constituye una fuente relevante de FLOPs en ninguna de sus cargas actuales; de incorporarse en el futuro un kernel nuevo, su precisión debería contrastarse de la misma manera antes de asumir que la convención de doble precisión se extiende a él. Las cargas del catálogo de GPU con precisión simple declarada (\texttt{gpu\_precision: fp32}, entre ellas varios kernels de la suite Rodinia) no están sujetas a esta restricción de presupuesto: su intensidad operacional se mide mediante Nsight Compute (sección \ref{sec:gpu-calibracion}), una vía de perfilado independiente que discrimina precisión de forma nativa y no comparte contadores de PMU de CPU con el mecanismo aquí descrito.

El diseño original de este instrumento, antes de confirmar la viabilidad de la medición directa, estimaba los FLOPs de cada ventana repartiendo un único total reportado por el propio programa al finalizar la corrida, en proporción a la fracción de instrucciones retiradas de cada ventana. Ese procedimiento --- y las razones por las que resultaba, en el mejor de los casos, una aproximación con un supuesto de homogeneidad no verificable en el caso general --- se documenta en detalle en el registro de cambios del proyecto (entrada ARC-27.1) como antecedente metodológico, pero no se conserva como mecanismo activo en el instrumento: dado que la plataforma y el catálogo de este trabajo no reclaman portabilidad más allá de lo descrito en la Tabla \ref{tab:plataforma}, y que la medición directa se confirmó disponible sin excepción en las 66 corridas de la campaña real (sección \ref{sec:validacion-flops}), mantener una vía de respaldo nunca ejercitada solo añadía una hipótesis sin sustento a la intensidad operacional sin ningún beneficio observado. Una ventana cuyos contadores de punto flotante no estén disponibles queda, en consecuencia, con intensidad operacional indefinida --- el mismo tratamiento que ya recibe cualquier otro contador core ausente o degradado en este instrumento (sección \ref{sec:multiplexacion}) --- en vez de recurrir a una estimación silenciosa.
