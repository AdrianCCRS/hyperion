...
En el modo de conteo empleado para los eventos de núcleo, cada lectura
representa el valor acumulado desde la habilitación o el último reinicio del
contador. La unidad de análisis no es, por tanto, una lectura individual, sino
la diferencia entre dos lecturas consecutivas, teniendo en cuenta posibles
desbordamientos. Las lecturas periódicas de \textit{uncore} empleadas en este
trabajo, en cambio, ya expresan conteos por intervalo y no deben volver a
diferenciarse.

La elección del período de muestreo es un compromiso: un período corto ofrece
mayor resolución temporal pero reduce el número de eventos acumulados por
ventana, degradando la relación señal--ruido de las métricas derivadas; un
período largo produce métricas más estables pero promedia intervalos que pueden
abarcar varias fases. Adicionalmente, el intervalo efectivo entre lecturas no
coincide exactamente con el período nominal solicitado, porque el hilo de
muestreo está sujeto a la planificación de un sistema operativo de propósito
general. Por ello, toda métrica derivada debe calcularse sobre el intervalo
realmente transcurrido, medido con un reloj monótono.
