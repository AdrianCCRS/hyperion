...
6  & \texttt{repetitions\_per\_combination} & 3 & Medido (este doc.), con límite documentado & Sección~\ref{sec:repetitions} -- mínimo operativo razonable, no garantía de detectar anomalías infrecuentes ni de precisión estadística para energía/EDP en Fase 4 \\
...
8  & Tolerancia cruzada GPU & 5\,\% & Parcial & Sección~\ref{sec:calibration-distribution} -- margen confirmado frente al ruido; detección end-to-end pendiente de permiso P4 \\
...
16 & Niveles de frecuencia F0--F4 & 5 puntos equiespaciados + REF & No medible hoy & Sección~\ref{sec:literatura} -- bloqueado por permiso P1/P4 pendiente, condición de cierre declarada \\
...
19 & Umbral de carga externa E08 & 1{,}0 & Justificado por convención, \textbf{no integrado en producción} & Sección~\ref{sec:literatura} -- valor razonable por convención, pero \texttt{run\_campaign\_preflight} no lo ejecuta y \texttt{run\_reduced\_preflight} (que sí lo incluye) nunca es invocado por \texttt{campaign.py} -- el chequeo no corre hoy en ninguna campaña real \\
20 & Rango de temperatura E02 & $[0,90]\,^\circ$C & Parcial, \textbf{no integrado en producción} & Sección~\ref{sec:literatura} -- 90\,$^\circ$C no coincide con el $T_{\text{case}}$ de 81\,$^\circ$C documentado para el SKU exacto (Xeon Gold 5315Y); además, \texttt{Manifest} no declara \texttt{package\_temperature\_c}, así que el chequeo -- aunque se invocara -- aprobaría siempre por sensor ausente \\
...
\textbf{2 quedan justificados por decisión metodológica o convención, pero
además con un hallazgo de integración pendiente} (\#16 F0-F4 bloqueado por
permiso pendiente; \#19 E08 por convención de sistemas, pero ni E08 ni E02
ni el preflight de GPU se ejecutan hoy en ninguna campaña real -- ver
