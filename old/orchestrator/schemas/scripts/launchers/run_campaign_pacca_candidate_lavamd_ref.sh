#!/bin/bash
# Caracterización REF de LavaMD: 1 kernel x 1 nivel x 10 repeticiones.
set -e -o pipefail

export PYTHONPATH="/home/latorresn/hyperion:${PYTHONPATH:-}"

srun -p GPU -w paccaA100 --nodes=1 --ntasks=1 --gres=gpu:1 --exclusive --time=00:30:00 bash -c '
...
python3 -m orchestrator.cli run-campaign \
  --manifest ~/hyperion/orchestrator/schemas/campaigns/campaign_pacca_candidate_lavamd_ref.yaml \
  --node-id pacca-a100 \
  --hostname "$(hostname)" \
  --reference-kernel-ref npb_mg \
  --campaign-timeout-seconds 1500
'
echo CANDIDATE_LAVAMD_SCRIPT_DONE
