...
| PRE-E06 | ☑ | Sin procesos ajenos corriendo activamente en delegated\_cpus, NO por cgroup. Bloqueante. **Corrección 2026-08-01:** `check_foreign_processes()` en sí ya estaba probado (test\_e02\_temperatura\_y\_e06\_procesos\_ajenos), pero recibía la lista de PIDs ajenos ya armada — no existía ningún código que escaneara `/proc` de verdad, y `run_reduced_preflight()` (que la invoca) nunca se llamaba desde `campaign.py`. `preflight.detect_foreign_affinity_pids()`/`campaign.py` cerraron ambos huecos (ARC-40), pero la primera versión filtraba por `Cpus_allowed`, que en la práctica marca como "ajeno" a casi cualquier daemon del sistema en reposo (rechazó las 6 combinaciones del primer piloto real sin ninguna contención real). **Corrección 2026-08-01 (ARC-44):** ahora usa el campo `processor` real de `/proc/<pid>/stat` + `state=='R'` — solo cuenta procesos corriendo AHORA MISMO en un core delegado, la señal físicamente correcta para contención de caché/ancho de banda. |
