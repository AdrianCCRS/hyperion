"""F4.2 -- preflight real contra felix (sin escribir nada en sysfs).

Correr con el entorno Conda hyperion-hpc activo y ~/hyperion-kernels como
cwd (catalog.yaml usa exec_path relativo). Resultados de environment/
...
def main() -> None:
    ...
    profile = node_profile.build_node_profile(
        env, [0, 1, 2, 3, 4, 5], node_id="felix-sc3", hostname=socket.gethostname()
    )
