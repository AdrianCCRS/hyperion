...
main( int argc, char** argv) 
{
    ...
    cudaSetDevice(0);  // PARCHE F1-GPU-014: un solo GPU en el nodo, device 1 no existe
		   ...
		   )
{
	...
	/* PARCHE F1-GPU-014: cudaBindTexture/texture<> retirados en CUDA 12 --
	   kmeansPoint ya recibe feature_d como parametro (lectura directa,
	   ver kmeans_cuda_kernel.cu). Nada que enlazar aqui. */

	/* copy clusters to constant memory -- API de simbolo por string
	   tambien retirada, se pasa el simbolo directamente (PARCHE F1-GPU-014) */
	...
	cudaDeviceSynchronize();  // PARCHE F1-GPU-014: cudaThreadSynchronize retirado
