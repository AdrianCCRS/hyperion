...
// PARCHE (F1-GPU-014): la API de texture references (texture<>, tex1Dfetch,
// cudaBindTexture) fue retirada en CUDA 12 -- reemplazada por lectura
// directa del arreglo global ya recibido como parametro del kernel.
// Semanticamente identico (la textura solo cacheaba el mismo arreglo
// read-only); unica diferencia es que el A100 usa su cache L1/L2 en vez
// del cache de textura dedicado, irrelevante para el perfil OI que se mide.
