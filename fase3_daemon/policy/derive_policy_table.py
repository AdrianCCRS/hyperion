...
def load_windows(windows_csv_paths: list[Path]) -> pd.DataFrame:
    ...
    para derivar política (§3.5 paso 1): quality_status="ok" en CPU,
    "gpu_telemetry" con etiqueta válida en GPU -- nunca corridas rechazadas.
    """
...
def compute_window_edp(df: pd.DataFrame) -> pd.Series:
    ...
    fabrica un EDP con energía no verificada.
    """
...
def derive_policy_for_class(
    ...
    return {
        ...
        "resolved_freq_khz" if device == "cpu" else "resolved_clock_mhz": (
            _median_observed_frequency(df, device, label, best_level)
        ),
...
def _median_observed_frequency(df: pd.DataFrame, device: str, label: str, level: str) -> float | None:
