/*
 * cublas_dgemm_pedantic_bench.cu -- variante de old/kernels/gpu/cublas_dgemm_bench.cu
 * con CUBLAS_PEDANTIC_MATH forzado explicitamente. F1-GPU-012/013: el DGEMM
 * cuBLAS por defecto (gpu_dgemm_n4096/gpu_dgemm_calibration, mismo binario
 * original) enruta a un kernel Tensor Core (cutlass_80_tensorop_d884gemm,
 * ARC-76) sin que se le pida -- confirmado con ncu (DMMA>0, DFMA~0). La
 * documentacion de cuBLAS (CUDA 12.0, "Tensor Core Usage") indica que
 * CUBLAS_PEDANTIC_MATH desactiva el uso de Tensor Core. Esta variante lo
 * configura explicitamente via cublasSetMathMode() antes de cualquier
 * llamada a cublasDgemm, para intentar recuperar un GEMM FP64 escalar
 * optimizado por el proveedor (no un kernel propio) como candidato
 * compute-bound. Es configuracion de una biblioteca existente, no un GEMM
 * escrito a mano.
 */
#include <cublas_v2.h>
...
int main(int argc, char** argv) {
    ...
    // Unica diferencia real frente a cublas_dgemm_bench.cu: forzar modo
    // pedantico para desactivar el enrutamiento automatico a Tensor Core.
    ...
    std::printf("\n cuBLAS DGEMM Benchmark PEDANTIC (GPU)\n\n");
