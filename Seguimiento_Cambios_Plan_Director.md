...
## F2-CPU-007 - Análisis de potencia estadística y refinamiento condicionado de la política memory-bound (2026-09-22)

- **Motivo:** la tabla clase-frecuencia (F2-CPU-006) reporta `no_actuar` en ambas clases, pero el autor pidió discutir si ese resultado esconde margen de mejora en memory-bound específicamente, y si hay algo defendible que hacer al respecto antes de cerrar.
- **Diagnóstico de potencia (script `fase2_clasificador/analysis/cpu_policy_by_family.py`):** con la familia (no el kernel) como unidad, ningún nivel mejora el EDP en memory-bound bajo *leave-one-family-out*: el ruido de medición (sd relativa mediana 1.05 %) es 5 a 10 veces menor que la dispersión entre familias (2.75 % en F0 hasta 15.53 % en F3), así que la varianza es heterogeneidad física real, no error instrumental. El efecto mínimo detectable al 80 % de potencia con 19 familias es 1.75 % en F0, 3.38 % en F1 y 6.05 % en F2: el efecto mínimo relevante de 1 % declarado en F2-CPU-006 era indetectable por diseño.
- **Señal condicional identificada:** dentro de memory-bound, las familias con `stall_mem_ratio` (ya en el vector de 6 del clasificador) mayor o igual a 0.30 ganan 3.03 % en F1 y 2.70 % en F2 en promedio; las de razón baja pierden 5-28 %. Con solo 6 familias en el subgrupo alto, el MDE(80 %) es ~5 %, insuficiente para demostrar el efecto con los datos de entrenamiento (`fase2_clasificador/analysis/cpu_policy_conditional.py`, regla anidada *leave-one-family-out*: ganancia realizada $-1.05\%$, IC95 $[-2.07\%,-0.17\%]$, no confirma).
- **V2, filtro barato (`fase2_clasificador/analysis/cpu_alpha_predict.py`):** predecir $\alpha$ (fracción de tiempo insensible al reloj) desde `stall_mem_ratio` por regresión de una variable, validado *leave-one-family-out* estricto: R² LOFO = 0.39, ganancia realizada media +0.51 % (IC95 $[0{,}0\%; 1{,}44\%]$). No confirma por sí solo pero no refuta, así que se decidió invertir en la prueba sellada V1.
- **V1, prueba sellada con familias nuevas:** se congeló la regla "actuar en F2 si `stall_mem_ratio`(REF) ≥ 0.30" **antes** de medir. Se construyeron 21 kernels nuevos de RAJAPerf (`scripts/pacca/rajaperf_v1_screen_adapters/`, catalogados en `fase1_telemetria/catalog/catalog.yaml`); 10 de ellos resultaron ser la misma familia que kernels ya usados en entrenamiento (el patrón `derive_kernel_family` funde cualquier `cpu_rajaperf_basic_*/_lcals_*/_polybench_*` con `rajaperf_basic/lcals/polybench`) y se descartaron de la prueba sin usarse. De los 11 kernels con familia genuinamente nueva, un tamizaje en REF (jobs 7541 y 7547, campañas `pacca_cpu_v1_screen_20260922` y `_r2`) midió `stall_mem_ratio`: solo 6 de 11 (objetivo: 8) quedaron del lado de actuar. Se decidió medir de todas formas con la potencia disponible (aprobación explícita del autor). El barrido REF/F0/F1/F2 × 3 repeticiones (job 7548, campaña `pacca_cpu_v1_edp_20260922`, 132/132 corridas aceptadas) dio **resultado refutado**: ganancia media del EDP en F2 sobre las 6 familias que actúan = $-25.76\%$ (empeora), IC95 $[-50.61\%,-6.84\%]$ (no toca cero), con `algorithm_histogram` (la razón de stall más alta del conjunto, 0.97) empeorando 85.6 %. Solo `apps_pressure` mostró la ganancia esperada (+3.5 %). Se cumplen las tres condiciones de refutación declaradas de antemano.
- **Post-mortem de V1 (2026-09-22):** la refutación de V1 tenía una causa identificable de tamaño de problema. Los 11 kernels se midieron con `--repfact` (tamaño por defecto de RAJAPerf, que cabe en la LLC de 8 MiB del nodo), mientras que las familias de streaming de entrenamiento usan `--memory-touched 125829120` (10x LLC). En consecuencia, el `stall_mem_ratio` alto que activaba la regla provenía de latencia de L2/L3 o de atómicos, no de tráfico real a DRAM: `algorithm_histogram` tenía razón de stall 0.97 con `mpki` 0.0, y `comm_halo_packing` 0.34 con `mpki` 0.008. En las familias de entrenamiento esa confusión no era visible porque las 6 con razón de stall mayor o igual a 0.30 tenían todas `mpki` mayor o igual a 20.
- **V1b, segunda prueba sellada con la física corregida:** se redimensionaron las 15 familias a `--memory-touched 125829120` (entradas `_llc10x` del catálogo, mismos wrappers y mismos checksums que V1) y se congeló la regla corregida con un segundo umbral: actuar en F2 si `stall_mem_ratio`(REF) ≥ 0.30 **y** `mpki`(REF) ≥ 20, ambos tomados de las familias de entrenamiento. Tamizaje (job 7551, campaña `pacca_cpu_v1b_screen_20260922`, 45/45 corridas aceptadas): el filtro de `mpki` excluyó correctamente los dos falsos positivos de V1 (`algorithm_histogram`, `mpki` 17.0; `apps_nodal_accumulation_3d`, `mpki` 4.5) y dejó 4 familias del lado de actuar. Barrido REF/F0/F1/F2 × 3 repeticiones (job 7552, campaña `pacca_cpu_v1b_edp_20260922`, 171/180 aceptadas; las 9 rechazadas son F0/F1/F2 de `algorithm_reduce_sum` por piso de ventanas, factor `I10`, sin EDP asociado que pudiera sesgar su exclusión). **Resultado refutado** otra vez: ganancia media en F2 sobre las 3 familias con dato = $-4.45\%$, IC95 $[-12.32\%; 5.02\%]$ (incluye cero), `algorithm_scan` pierde 12.32 % (más del 5 % declarado). Se cumplen 2 de los 3 criterios de refutación. Solo `apps_pressure` repitió el signo esperado (+5.02 %), y `algorithm_memcpy`, la familia con más tráfico medido a DRAM del conjunto (`mpki` 70.3), empeoró 6.06 %.
- **Explicación física del cierre (2026-09-22), y re-verificación pedida por el plan §3.4:** el plan pide re-verificar con la campaña reorganizada el dato previo de que "reducción de reloj 4x baja la potencia solo 28 %". Hecho, y el valor real es peor: sobre los 28 kernels memory-bound de la campaña final, $P(f)/P_{REF} = 0.803 + 0.058\,f$ (f en GHz), es decir que el **81 % de la potencia de paquete a 3.2 GHz no escala con la frecuencia**; bajar el reloj 4x ahorra **14.6 %** de potencia mientras el tiempo sube 1.83x. Como el EDP castiga el tiempo al cuadrado, se despeja un umbral cerrado sobre la fracción de tiempo insensible al reloj: harían falta valores de 0.857 para ganar algo en F1 y 0.906 para ganar 1 %, mientras que el máximo del catálogo es 0.881 (`rajaperf_stream`) y **ninguna de las 19 familias alcanza el umbral de F2**. La región objetivo está vacía en este procesador, lo que explica hacia atrás el `no_actuar` agregado y las dos refutaciones. Derivación completa y tabla de umbrales en `recordatorios/umbral_alpha_dvfs_cpu_para_fases_3_4_20260922.md` (deliberadamente fuera del capítulo: el libro ya cierra por el piso de potencia y una segunda parametrización de $\alpha$ solo confundiría).
- **Cierre:** la política de frecuencia CPU queda como en F2-CPU-006 (`no_actuar` en ambas clases), sin cambios en `policy_cpu.json["policy"]`. Se añaden al mismo archivo las claves `statistical_power_analysis` (MDE por nivel, descomposición de varianza, auditoría de sesgos REF-vs-F0 y campaña main-vs-sweep), `conditional_refinement_attempt_v1` y `conditional_refinement_attempt_v1b` (regla congelada, cobertura del tamizaje, resultado por familia, veredicto). Evidencia completa en `docs/libro/datos/cpu_calidad_30fam/politica/v1_prueba_sellada/` y `.../v1b_prueba_sellada/`. En el libro, `docs/libro/secciones/03_resultados.tex` incorpora el análisis de potencia del diseño (§ Qué tamaño de efecto podía detectar este diseño) y una mención de dos frases a las pruebas externas, sin narrativa de depuración, según el criterio del autor de incluir solo metodología final y resultado final.
- **Decisión del autor:** no seguir intentando reglas condicionadas adicionales sobre memory-bound; el hallazgo negativo (heterogeneidad con dirección conocida, mecanismo identificado, y ahora además explicado por el piso de potencia medido) queda cerrado, no como limitación sin explorar. Un tercer ajuste de umbral sobre los mismos kernels sería buscar hasta que algo salga, prohibido por el protocolo declarado.
