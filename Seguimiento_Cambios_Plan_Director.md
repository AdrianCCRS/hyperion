...
- **Cierre:** la política de frecuencia CPU queda como en F2-CPU-006 (`no_actuar` en ambas clases), sin cambios en `policy_cpu.json["policy"]`. Se añaden al mismo archivo las claves `statistical_power_analysis` (MDE por nivel, descomposición de varianza, auditoría de sesgos REF-vs-F0 y campaña main-vs-sweep) y `conditional_refinement_attempt_v1` (regla congelada, cobertura del tamizaje, resultado por familia, veredicto), documentadas también en `docs/libro/secciones/03_resultados.tex` (§ Potencia del diseño y un intento de refinamiento). Evidencia completa en `docs/libro/datos/cpu_calidad_30fam/politica/v1_prueba_sellada/`.
- **Decisión del autor:** no seguir intentando reglas condicionadas adicionales sobre memory-bound; el hallazgo negativo (heterogeneidad con dirección conocida, mecanismo identificado, no demostrable con el catálogo actual) queda como trabajo futuro con el diseño experimental ya especificado, no como limitación sin explorar.
